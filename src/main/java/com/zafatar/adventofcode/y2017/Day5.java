...
public class Day5 implements Day {
	...
	public void solvePart1() {
		...
		while (index < this.input.size()) {
			...
			this.input.set(index, value+1);    // Increase the current element, +1.

			index += value;                    // calculate new index, jump.
			this.steps++;                      // count steps.
		}

		System.out.println("Day #5 - Part #1: " + this.steps);
	}
	...
	public void solvePart2() {
		...
		while (index < this.input.size()) {
			...
			// New rule.
			if (value >= 3) {
				this.input.set(index, value-1);  // Decrease the current element, -1.
			}
			else {
				this.input.set(index, value+1);  // Increase the current element, +1.
			}
		...
		System.out.println("Day #5 - Part #2: " + this.steps);
	}
